...
class LearnedWordTrainer(
    ...
    fun getNextQuestion(): Question? {
        ...
        if (listOfChooseWords.size < quantityOfWord) {
            val listOfLearnedWords = dictionary.filter { it.correctAnswersCount >= countForRemember }.shuffled()
            val showListOfWord = listOfChooseWords + listOfLearnedWords.take(quantityOfWord - listOfChooseWords.size)
            val currentOriginalWord = listOfChooseWords.random()
            question = Question(
                variants = showListOfWord,
