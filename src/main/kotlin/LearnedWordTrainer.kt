...
class LearnedWordTrainer(
    ...
    fun getNextQuestion(): Question? {
        val notLearnedWords = dictionary.filter { it.correctAnswersCount < countForRemember }
        if (notLearnedWords.isEmpty()) return null
        val listOfChooseWords = notLearnedWords.take(quantityOfWord).shuffled()
    ...
    private fun loadDictionary(): List<Word> {
        ...
        wordsFile.readLines().forEach {
            ...
            if (splitLine.size == countForRemember) {
